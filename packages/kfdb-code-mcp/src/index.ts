...
import { buildToolList, handleToolCall } from "./tools.js";
import { RESPONSE_MAX_LENGTH, IS_BENCH_MODE } from "./config.js";

const SERVER_NAME = "kfdb-code-mcp";
...
if (!isHttp) {
  ...
  console.error(
    `${SERVER_NAME} running on stdio (${buildToolList().length} tools${IS_BENCH_MODE ? ", bench mode" : ""})`,
  );
} else {
  ...
  app.listen(port, () => {
    ...
    console.log(`Tools: ${buildToolList().length}`);
  });
}
